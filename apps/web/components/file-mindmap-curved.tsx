...
export function FileMindMapCurved({ files, fullscreen = false, onToggleFullscreen }: Props) {
  ...
  const treeData = useMemo(() => {
    ...
    const walk = (node: FileHierarchyNode, depth: number) => {
      if (node.type === "folder") folderCount += 1;
      maxDepth = Math.max(maxDepth, depth);
      node.children?.forEach((c) => walk(c, depth + 1));
    };
    walk(root, 0);
    setStats({ files: root.fileCount, folders: Math.max(0, folderCount - 1), depth: maxDepth });
    return root;
  }, [files]);
  ...
  useEffect(() => {
    ...
    // Initial state: everything collapsed (only root visible).
    collapseDeep(root);
    ...
    const update = (source: any, animate: boolean) => {
      ...
      linkSel
        ...
        .attr("class", "link")
        .attr("fill", "none")
        .attr("stroke", "rgba(15, 23, 42, 0.22)")
      ...
      nodeEnter
        ...
        .on("mouseenter", (event, d: any) => {
          ...
          const path = d.data.path ? `<div style=\"font-size:11px;opacity:.85\">${d.data.path}</div>` : "";
          const info =
            d.data.type === "folder"
              ? `<div style=\"font-size:11px;opacity:.85\">${d.data.fileCount} files</div>`
              : "";
          tooltip.innerHTML = `<div style=\"font-weight:700\">${d.data.name}</div>${path}${info}`;
        })
        .on("mouseleave", () => {
